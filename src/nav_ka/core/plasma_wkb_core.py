...
def compute_group_delay_from_phase(
    ...
    phase_shift_rad:
        相位数组，第一维必须对应频率轴。
        - shape=(nf,)      -> 返回 shape=(nf,)
        - shape=(nf, nt)   -> 返回 shape=(nf, nt)

    返回
    ----
    tau_g_s:
        群时延，满足 tau_g = -d(phi)/d(omega)。
    """
    freq = np.asarray(frequencies_hz, dtype=float).reshape(-1)
    phase = np.asarray(phase_shift_rad, dtype=float)

    if freq.size < 3:
        raise ValueError("frequencies_hz 至少需要 3 个点才能稳定计算群时延。")
    if not np.all(np.diff(freq) > 0):
        raise ValueError("frequencies_hz 必须严格递增。")
    if phase.shape[0] != freq.size:
        raise ValueError("phase_shift_rad 的第一维长度必须与 frequencies_hz 一致。")

    omega = 2.0 * np.pi * freq
    return -np.gradient(phase, omega, axis=0, edge_order=2)


def run_wkb_frequency_sweep(
    frequencies_hz: np.ndarray,
    z_grid: np.ndarray,
    Ne_matrix: np.ndarray,
    nu_en: float | np.ndarray,
    ...
    for i, f_hz in enumerate(freq):
        if verbose:
            print(f">>> WKB 多频扫描 {i + 1}/{freq.size}: {f_hz / 1e9:.3f} GHz")

        amp_t, phi_t, hp_t = run_wkb_analysis(
            f_EM=float(f_hz),
            z_grid=z_grid,
            Ne_matrix=Ne_matrix,
            nu_en=nu_en,
        ...
        amplitude_list.append(amp_t)
        phase_list.append(phi_t)
        transmission_list.append(hp_t)

    amplitude = np.asarray(amplitude_list, dtype=float)
    phase_shift_rad = np.asarray(phase_list, dtype=float)
    transmission_coeff = np.asarray(transmission_list)
    attenuation_dB = 20.0 * np.log10(amplitude + 1e-30)
    group_delay_s = compute_group_delay_from_phase(freq, phase_shift_rad)
...
def summarize_wkb_frequency_sweep(
    frequencies_hz: np.ndarray,
    ...
    atten = np.asarray(attenuation_dB, dtype=float)
    phi = np.asarray(phase_shift_rad, dtype=float)
    tau = np.asarray(group_delay_s, dtype=float)

    if amp.shape[0] != freq.size:
        raise ValueError("amplitude 第一维长度必须与 frequencies_hz 一致。")

    def _series_summary(x: np.ndarray) -> dict[str, list[float]]:
        return {
            "min": np.min(x, axis=1).astype(float).tolist(),
            "max": np.max(x, axis=1).astype(float).tolist(),
            "median": np.median(x, axis=1).astype(float).tolist(),
            "p05": np.percentile(x, 5.0, axis=1).astype(float).tolist(),
            "p95": np.percentile(x, 95.0, axis=1).astype(float).tolist(),
        }

    return {
        "frequencies_hz": freq.astype(float).tolist(),
        "amplitude": _series_summary(amp),
        "attenuation_dB": _series_summary(atten),
        "phase_shift_rad": _series_summary(phi),
        "group_delay_s": _series_summary(tau),
    }
